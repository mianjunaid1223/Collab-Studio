...
export function AudioVisualCanvas({ project, contributions, onContribute, user, activeWaveform, activeBPM }: CanvasProps) {
    ...
    return (
        <div className="w-full flex flex-col items-center justify-center gap-4">
             <div 
                className="grid bg-card border-2 border-border shadow-2xl relative overflow-hidden aspect-square w-full h-full max-w-none"
                style={{
            ...
            >
                {Array.from({ length: COLS * ROWS }).map((_, i) => {
                    ...
                    return (
                        ...
                        >
                            <div className={cn(
                                'w-3/4 h-3/4 rounded-sm transition-all duration-300',
                                colorClass,
                                {
